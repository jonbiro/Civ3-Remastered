...
- Do not force a newer runtime solely to adopt a package major version when the game engine does not support that runtime.
...
- The test suite remains on xUnit v2 2.9.3. Current xUnit v3 3.2.2 requires .NET 9, while Godot 4.7 uses .NET 8. We will not move the tests to a newer runtime than the game merely to adopt a newer test-framework major version.
...
The export pipeline separately produces Windows, Linux, and macOS builds using Godot 4.7.1 .NET export templates.
